...
Server.ignoreEmotes = {};
try {
	Server.ignoreEmotes = JSON.parse(FS(`server/impulse/database/ignoreemotes.json`).readIfExistsSync());
} catch (e) {}
...
function loadEmoticons() {
	try {
		emoticons = JSON.parse(FS(`server/impulse/database/emoticons.json`).readIfExistsSync());
		emoteRegex = [];
...
function saveEmoticons() {
	FS(`server/impulse/database/emoticons.json`).writeSync(JSON.stringify(emoticons));
	emoteRegex = [];
	for (let emote in emoticons) {
...
exports.commands = {
	...
	emoticon: {
		add: function (target, room, user) {
			...
			if (Rooms(`staff`)) Rooms(`staff`).add(`|raw|${Server.nameColor(user.name, true)} has added the emoticon ${Chat.escapeHTML(targetSplit[0])}: <img src="${targetSplit[1]}" width="${size}" height="${size}">`);
		},
		...
		del: function (target, room, user) {
			...
			if (Rooms(`staff`)) Rooms(`staff`).add(`|raw|${Server.nameColor(user.name, true)} has removed the emoticon ${Chat.escapeHTML(target)}.`);
		},
		...
		ignore: function (target, room, user) {
			...
			FS(`server/impulse/database/ignoreemotes.json`).writeSync(JSON.stringify(Server.ignoreEmotes));
