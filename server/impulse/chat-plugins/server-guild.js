...
let database = new sqlite3.Database('server/impulse/database/leagues.db', function () {
	database.run("CREATE TABLE IF NOT EXISTS points (date INTEGER, userid TEXT, league TEXT, points INTEGER, reason TEXT)");
});

let leagues = {};
try {
	leagues = JSON.parse(fs.readFileSync('server/impulse/database/leagues.json', 'utf8'));
} catch (e) {
	if (e.code !== 'ENOENT') throw e;
}

function save() {
	if (Object.keys(leagues).length < 1) return fs.writeFileSync('server/impulse/database/leagues.json', JSON.stringify(leagues));
	let data = "{\n";
	...
	data += "\n}";
	fs.writeFileSync('server/impulse/database/leagues.json', data);
}
...
exports.commands = {
	...
	guilds: {
		create: function (target, room, user) {
			if (!this.can('guild')) return false;
